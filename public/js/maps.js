...
const MapData = (() => {
  ...
  const nexus = {
    ...
    warps: [
      ...
      { x: 12, y: 21, toMap: 'rift', toX: 12, toY: 1 },
    ],
    chests: [
  ...
  (function buildNexus() {
    ...
    for (let y = 0; y < nexus.height; y++) {
      ...
      for (let x = 0; x < nexus.width; x++) {
        ...
        // South exit to Temporal Rift
        if (y === 21 && x >= 11 && x <= 13) t = T.NEBULA_GLOW;
  ...
  // Temporal Rift map (24x20)
  const rift = {
    name: 'Temporal Rift',
    width: 24,
    height: 20,
    encounterRate: 0.15,
    enemyPool: ['chronoPhantom', 'temporalGuardian'],
    tiles: [],
    npcs: [],
    warps: [
    ...
    chests: [
      ...
      { x: 20, y: 9, item: 'temporalArmor', opened: false },
    ],
    bossSpawn: { x: 12, y: 17 },
    playerStart: { x: 12, y: 1 },
  };
  ...
  (function buildRift() {
    const m = [];
    for (let y = 0; y < rift.height; y++) {
      m[y] = [];
      for (let x = 0; x < rift.width; x++) {
        let t = T.TEMPORAL_STONE;
        ...
        // Main north-south corridor
        if (x >= 11 && x <= 13 && y >= 0 && y <= 12) t = T.RIFT_GLOW;

        // West wing corridor
        if (y >= 7 && y <= 9 && x >= 1 && x <= 11) t = T.RIFT_GLOW;
        // West wing chamber
        if (x >= 1 && x <= 6 && y >= 6 && y <= 11) t = T.RIFT_GLOW;

        // East wing corridor
        if (y >= 7 && y <= 9 && x >= 13 && x <= 22) t = T.RIFT_GLOW;
        // East wing chamber
        if (x >= 17 && x <= 22 && y >= 6 && y <= 11) t = T.RIFT_GLOW;

        // Temporal pools in wings
        if (x >= 2 && x <= 3 && y >= 7 && y <= 8) t = T.WATER;
        if (x >= 20 && x <= 21 && y >= 7 && y <= 8) t = T.WATER;

        // Boss chamber (south)
        if (x >= 4 && x <= 19 && y >= 12 && y <= 18) t = T.RIFT_GLOW;
        if (x >= 4 && x <= 19 && y === 11) t = T.WALL;
        if ((x === 4 || x === 19) && y >= 11 && y <= 18) t = T.WALL;
        if (x >= 11 && x <= 13 && y === 11) t = T.RIFT_GLOW; // entrance

        // Inner boss floor
        if (x >= 6 && x <= 17 && y >= 14 && y <= 18) t = T.FLOOR;

        // Chests
        if (x === 3 && y === 9) t = T.CHEST;
        if (x === 20 && y === 9) t = T.CHEST;
  ...
  const realm = {
    ...
    warps: [
      ...
      { x: 11, y: 19, toMap: 'prism', toX: 12, toY: 1 },
    ],
    chests: [
  ...
  (function buildRealm() {
    ...
    for (let y = 0; y < realm.height; y++) {
      ...
      for (let x = 0; x < realm.width; x++) {
        ...
        // South exit to Prismatic Void
        if (y === 19 && x >= 10 && x <= 12) t = T.FRACTURE_GLOW;
        if (y === 18 && x >= 10 && x <= 12) t = T.FRACTURE_GLOW;
  ...
  // Prismatic Void map (24x20)
  const prism = {
    name: 'Prismatic Void',
    width: 24,
    height: 20,
    encounterRate: 0.15,
    enemyPool: ['prismPhantom', 'spectraSentinel'],
    tiles: [],
    npcs: [],
    warps: [
    ...
    chests: [
      ...
      { x: 19, y: 10, item: 'prismArmor', opened: false },
    ],
    bossSpawn: { x: 12, y: 17 },
    playerStart: { x: 12, y: 1 },
  };
  ...
  (function buildPrism() {
    const m = [];
    for (let y = 0; y < prism.height; y++) {
      m[y] = [];
      for (let x = 0; x < prism.width; x++) {
        let t = T.PRISMATIC_STONE;
        ...
        // Main north-south corridor
        if (x >= 11 && x <= 13 && y >= 0 && y <= 12) t = T.PRISMATIC_GLOW;

        // West wing corridor
        if (y >= 8 && y <= 10 && x >= 1 && x <= 11) t = T.PRISMATIC_GLOW;
        // West wing chamber
        if (x >= 1 && x <= 7 && y >= 7 && y <= 12) t = T.PRISMATIC_GLOW;

        // East wing corridor
        if (y >= 8 && y <= 10 && x >= 13 && x <= 22) t = T.PRISMATIC_GLOW;
        // East wing chamber
        if (x >= 16 && x <= 22 && y >= 7 && y <= 12) t = T.PRISMATIC_GLOW;

        // Prismatic pools in wings
        if (x >= 2 && x <= 3 && y >= 8 && y <= 9) t = T.WATER;
        if (x >= 20 && x <= 21 && y >= 8 && y <= 9) t = T.WATER;

        // Boss chamber (south)
        if (x >= 5 && x <= 18 && y >= 12 && y <= 18) t = T.PRISMATIC_GLOW;
        if (x >= 5 && x <= 18 && y === 12) t = T.WALL;
        if ((x === 5 || x === 18) && y >= 12 && y <= 18) t = T.WALL;
        if (x >= 11 && x <= 13 && y === 12) t = T.PRISMATIC_GLOW; // entrance
